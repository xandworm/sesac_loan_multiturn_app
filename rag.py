...
# embedding 객체 정의
@st.cache_resource      # 덩치가 큰 함수를 매 번 메모리에 올리면 비효율적이기 때문에 캐싱되도록.
def get_embedding(): 
...
def get_vectorstore(
    ...
    if vectorstore_exists: 
        ...
        print("⚠️ vectorstore가 존재하지 않습니다. 새로 생성합니다.")
    ...
    for pdf_path in PDF_PATH: 
        ...
        # 어느 은행 문서인지 metadata에 추가
        bank_name = os.path.basename(pdf_path).replace("전세자금대출_상품설명서.pdf", "")
    ...
    if not all_docs: 
        raise FileNotFoundError(                        # raise: 예외를 발생시켜서 코드를 중단시킴
            "PDF 파일을 찾을 수 없습니다."
            "data 폴더에 상품설명서.pdf를 저장해주세요."
        )
...
# 검색된 컨텍스트를 하나의 문자열로 합치는 함수 정의
def build_context_string(contexts_only_text: list[str]) -> str: 
    """검색된 chunk들을 개행으로 연결하여 프롬프트 삽입용 문자열로 반환"""
...
if __name__ == "__main__": 
    ...
    # - Python에서 명령줄 인자(command-line arguments)를 받는 방법
    input_data = (
        sys.argv[1]
        if len(sys.argv) > 1
