...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ 페이지 설정 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
st.set_page_config(
    page_title="전세자금대출 AI 컨설턴트", 
    page_icon="🏠",
    layout="centered"
)

st.title("전세자금대출 AI 컨설턴트")
st.caption(
...
if "messages" not in st.session_state: 
    st.session_state.messages = []                  # LangChain 메세지 객체 리스트 (모델 전달용)
...
with st.sidebar: 
    ...
    credit_score = st.number_input(
        "신용점수(점)", min_value=0, max_value=1_000, value=0, step=10
    )

    existing_loan = st.number_input(
        "희망 전세금", min_value=0, max_value=200_000, value=0, step=10
    )
    ...
    if st.button("📊 프로필 기반 진단 시작", use_container_width=True):
        if annual_income > 0:
            ...
            # 모델 전달용 메세지 (RAG context 포함)
            model_msg = (
                f"[입력_고객프로필]\n{input_data}\n\n"
                f"[RAG_검색문서]\n{context_str}\n\n"
                "[응답_요청]\n"
                "- RAG 근거를 우선 사용하고 불확실한 내용은 '확인 필요'로 표시\n"
                "- turn 별 고정 포맷을 지켜 1 turn 답변 생성"
            )

            # UI 표시용 메세지 (context 제외)
...
# 사용자 입력 처리
if prompt := st.chat_input("예) 희망은행=신한, 대출목적=오피스텔 전세계약(갱신), 연소득=4,200만원, 신용점수=742점, 희망 전세금=3억1천만원, 희망 대출액=2억4천만원"): 
    # RAG 검색 결과 갖고오기
    _, contexts_text = search_context(prompt)
    context_str = build_context_string(contexts_text)

    # 모델 전달용 user_prompt 작성
    model_prompt = (
                f"[입력_고객프로필]\n{prompt}\n\n"
                f"[RAG_검색문서]\n{context_str}\n\n"
                "[응답_요청]\n"
